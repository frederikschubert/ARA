...
class SAC(RlAlgorithm):
    ...
    def optim_initialize(self, rank=0):
        ...
        if self.fixed_alpha is None:
            self._log_alpha = torch.zeros(1, requires_grad=True, dtype=torch.float32, device=self.agent.device)
            self._alpha = torch.exp(self._log_alpha.detach())
            self.alpha_optimizer = self.OptimCls((self._log_alpha,),
                lr=self.learning_rate, **self.optim_kwargs)
        else:
            self._log_alpha = torch.tensor([np.log(self.fixed_alpha)], dtype=torch.float32, device=self.agent.device)
            self._alpha = torch.tensor([self.fixed_alpha], device=self.agent.device)
    ...
    def loss(self, samples):
        ...
        agent_inputs, target_inputs, action, valid, returns, dones = buffer_to(
            (samples.agent_inputs, samples.target_inputs, samples.action, valid, samples.return_, samples.done_n), device=self.agent.device)
        ...
        y = (self.reward_scale * returns +
            (1 - dones.float()) * disc * target_value)
    ...
    def append_opt_info_(self, opt_info, losses, grad_norms, values):
        ...
        opt_info.q1GradNorm.append(q1_grad_norm.clone().detach().item())
        opt_info.q2GradNorm.append(q2_grad_norm.clone().detach().item())
        opt_info.piGradNorm.append(pi_grad_norm.clone().detach().item())
        opt_info.q1.extend(q1[::10].cpu().numpy())  # Downsample for stats.
        opt_info.q2.extend(q2[::10].cpu().numpy())
        opt_info.piMu.extend(pi_mean[::10].cpu().numpy())
        opt_info.piLogStd.extend(pi_log_std[::10].cpu().numpy())
