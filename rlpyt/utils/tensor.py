...
def select_at_indexes(indexes: torch.Tensor, tensor: torch.Tensor):
...
def to_onehot(indexes: torch.Tensor, num: int, dtype=None):
...
def from_onehot(onehot: torch.Tensor, dim=-1, dtype=None):
...
def valid_mean(tensor: torch.Tensor, valid=None, dim=None):
...
def restore_leading_dims(tensors, lead_dim, T=1, B=1) -> Union[torch.Tensor, Iterable[torch.Tensor]]:
