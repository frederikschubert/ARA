...
from rlpyt.distributions.utils import stable_softmax
import torch
...
class StochasticEpsilonGreedy(EpsilonGreedy):
    def __init__(self, temperature: float = 1.0, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.temperature = temperature

    def sample(self, q):
        policy = stable_softmax(q, temperature=self.temperature).cpu()
        arg_select = torch.multinomial(policy, num_samples=1)
        mask = torch.rand(arg_select.shape) < self._epsilon
        arg_rand = torch.randint(low=0, high=policy.shape[-1], size=(mask.sum(),))
        arg_select[mask] = arg_rand
        return arg_select
